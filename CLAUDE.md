...
- **Sports tools** (`tools/sports/`): `find_player`, `get_sport_catalog`, `get_leaderboard`, `get_player_season_stats`, `get_player_game_log`, `compare_players`. Args are coerced through `tools.utils.ts` because small models send numbers as strings and bare strings for arrays. Every list is capped (`LEADERBOARD_LIMIT`, `GAME_LOG_LIMIT`) — an uncapped result will bury the question in the context window.
...
- **Prompting**: `SYSTEM_PROMPT` in `chat.constants.ts` spells out both tool families and Sleeper's id lookup chain (username → user_id → league_id), because small models otherwise invent league ids. `SEASON_CONTEXT` is appended per request with the season and week resolved from the NFL catalog: telling the model the year is the only reliable fix for "last season" — instructing it to look the year up does not work at this size. Tune prompts there, not inline.
