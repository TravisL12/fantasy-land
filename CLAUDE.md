...
- **Baseball tools** (`tools/baseball/`): `get_pitcher_starts` (every "who is pitching" question — a one-day range for tonight, `confirmedOnly` for announced starts only, `minStarts: 2` for two-start weeks; it absorbed `get_probable_pitchers`, which asked the same service for the same window), `get_matchup_ratings`, `compare_teams` (team head-to-head: the games two teams played each other with scores and the series record, plus both teams' production over the same window — passing both dates measures team stats over the interval via `byDateRange` instead of season-to-date), `get_player_status`. They default to `mlb` rather than `nfl` and go through the `LeagueDataProvider` capability, so they return a clear "not wired up for this sport" message elsewhere. Date windows are validated and capped by `resolveDateRange` (`sports.utils.ts`).
