...
- **Warm-up**: the first question otherwise pays four cold starts at once — Ollama loading the weights, prefilling the system prompt and tool schemas, spawning the MCP stdio servers and fetching the season catalogs. `ChatService.warmUp()` pays all four up front: on boot (`OnApplicationBootstrap`) and again on `GET /chat/status`, which the chat page requests on mount, so the load happens while the question is being typed. It sends the **exact prefix a real turn sends** (same system prompt, tools, `think` and options) with `num_predict: 1`, so Ollama's prompt cache serves it and the real turn only prefills the question. `OLLAMA_KEEP_ALIVE` (default `30m`, `-1` to pin) goes on every request, because Ollama's own 5-minute default unloads the model and the cached prefix with it. It never throws, is deduped, is skipped mid-conversation and is rate-limited to `WARM_MIN_INTERVAL_MS`; `OLLAMA_WARMUP=false` turns it off. Measured on this box, a cold first question spends its whole prompt on prefill; warmed, the same prompt prefills in ~3% of that. Note that Ollama still reports the **full** `prompt_eval_count` on a cache hit — `prompt_eval_duration` is the only honest signal that the prefix was reused. The warm-up deliberately sends **no user message**, so the cached prefix ends exactly at the system boundary; a warm-up carrying a dummy question caches a prefix the real question diverges from early and recovers only about half the time.
